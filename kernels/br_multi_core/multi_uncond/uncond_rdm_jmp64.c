...
static int create_loop(int max_depth)
{
	int i,j,k,rval;
	FILE  * fp_loop;
	char filename[100];
	...
	fprintf (fp_loop,"\tfor(k=0;k<iter;k++){\n");
        ...
        fprintf (fp_loop,"\t__asm__( \n");
	fprintf (fp_loop,"\t\".align 64\\n\\t\"\n");
	fprintf (fp_loop,"\t\"jmp LP_%06d\\n\\t\"\n",ran_array[0]);

        for(j=0;j<max_depth;j++){
		fprintf (fp_loop,"\t\".align 64\\n\\t\"\n");
		fprintf (fp_loop,"\t\"LP_%06d:\\n\\t\"\n",ordered_ran_array[j]);
		fprintf (fp_loop,"\t\"jmp LP_%06d\\n\\t\"\n",ran_array[ordered_target[j]+1]);
		fprintf (fp_loop,"\t\"xorq   %%rdx, %%rcx\\n\\t\"\n");
                }
	fprintf (fp_loop,"\t\"LP_%06d:\\n\\t\"\n",ran_array[max_depth]);
        fprintf (fp_loop,"\t);\n");
