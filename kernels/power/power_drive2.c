...
//        fprintf(stderr,"\n");
	...
	one_plus_eps = 1.000000000001;
	packed_1_plus_eps = _mm_set_pd (one_plus_eps, one_plus_eps);
// pin core affinity
...
*/
	...
	"movl %%edx, %0  \n\t"
	: "=r"(outval)
	: "r"(limit), "r"(msk1), "x"(packed_1_plus_eps)
	: "%r9", "%r10", "%r11", "%r12", "%r13", "%r14", "%r15", "%rdx", "%xmm2", "%xmm3", "%xmm4", "%xmm5", "%xmm6", "%xmm7"
	);
	pthread_exit(NULL);
}
