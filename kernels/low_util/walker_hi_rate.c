...
void usage()
	...
	fprintf(stderr," -CN  -m signifies core set size N indicates the number of cores starting at the run cpu to be used in a loop. \n");
...
int main(int argc, char ** argv)
{
	...
	off_t offset = 0;
	int len=1024000, iter=1000,mult=1,main_ret=0, num_cores=10,transitions=0;
	double iterations;
...
//	process input arguments
	...
	while ((c = getopt(argc, argv, "i:r:l:s:S:C:m:L:B:")) != -1) {
		switch(c) {
		case 'i':
			cpu = atoi(optarg);
			break;
...
//    create index array for "random" patterna
	...
	index_test = (int*)malloc(line_count*sizeof(int));
	if(index_test == NULL)
		{
		fprintf(stderr," failed to malloc index_test array for line_count of %d\n",line_count);
		err(1,"failed to malloc index_test");
		}
...
//	for(jj=0; jj< line_count; jj+=8) printf(", jj = %d, array[jj]-&array[0]/array_stride = %d\n",jj,(array[jj]-(size_t)&array[0])/array_stride);
	bad=0;
	pnt = array;
//	fprintf(stderr," array address = %p value = %lx\n",pnt,*pnt);
	while(bad < line_count + 4)
		{
		pnt = (size_t*)*pnt;
		element = (size_t) (pnt - array);
		element = element/var_size;
		bad++;
//		if(bad > line_count - 4)
//		fprintf(stderr,"index = %d,element = %ld, address = %p, value = %lx\n",bad,element,pnt,*pnt);
		}
