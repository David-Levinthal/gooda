...
// set buffer sizes and loop tripcounts based on memory level
	...
	mod_inner = len % inner;
	if(mod_inner != 0)
		err(1," buffer size len not divisible by inner, %d, mod_inner = %d",inner,mod_inner);
...
// run the triad
	...
	for(i=0;i<iter;i++){
		start = _rdtsc();
		bytes_per = triad(len,inner,a,b,index);
		stop = _rdtsc();
		run_time = stop - start;
		xx+=0.01;
