...
static int create_main(int max_depth)
{
	...
	fprintf (fp_main,"\tfor(k=0;k<j;k++){\n");
       ...
       fprintf (fp_main,"\t\".align 64\\n\\t\"\n");
		...
		fprintf (fp_main,"\t\".align 64\\n\\t\"\n");
		fprintf (fp_main,"\t\"movabs $0xFFFFFFFFFFFFFFFF, %%rax\\n\\t\"\n");
		fprintf (fp_main,"\t\"movabs $0xFFFFFFFFFFFFFFFF, %%rbx\\n\\t\"\n");
		fprintf (fp_main,"\t\"movabs $0xFFFFFFFFFFFFFFFF, %%rcx\\n\\t\"\n");
		fprintf (fp_main,"\t\"movabs $0xFFFFFFFFFFFFFFFF, %%rax\\n\\t\"\n");
		fprintf (fp_main,"\t\"movabs $0xFFFFFFFFFFFFFFFF, %%rbx\\n\\t\"\n");
		fprintf (fp_main,"\t\"mov $0xFFFFFFFF, %%ecx\\n\\t\"\n");
		fprintf (fp_main,"\t\"nopl 0x0(%rax)\\n\\t\"\n");
                }
        fprintf (fp_main,"\t);\n");
...
{
	int max_per_line,max_depth, rval;
	if(argc < 2){
		printf("generator needs one input number, argc = %d\n",argc);
		exit(1);
		}
	max_depth = atoi(argv[1]);
	printf ("  max_depth= %d, max_per_line = %d\n",max_depth);
